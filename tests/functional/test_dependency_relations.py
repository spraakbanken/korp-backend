"""Pytest tests for the Korp `/dependency_relations` endpoint."""
...
def dependency_relations_testcorpus(get_json: Callable, database_tables: Callable) -> Callable:
    """Return function returning JSON response for `/dependency_relations` to testcorpus.

    The returned function takes as its parameters a word, corpus (or corpora), possible additional query parameters and
    Korp configuration parameters. It returns the JSON response for `/dependency_relations` with the given parameters
    (and cache=false).
    """
    ...
    def _dependency_relations_testcorpus(
        ...
        return get_json("/dependency_relations", params=query_params, config=config)

    return _dependency_relations_testcorpus


class TestDependencyRelations:
    """Tests for `/dependency_relations`."""
    ...
    def test_dependency_relations_simple(
        ...
        """Test `/dependency_relations` with the given word and corpora."""
